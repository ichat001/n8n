...
install_n8n() {
    ...
    log "安装 n8n（本地依赖）..."
    cd "$USER_HOME/n8n-serv00"
    npm install n8n sqlite3 || error "n8n 安装失败"
}
...
start_n8n() {
    ...
    if check_status; then
        ...
        log "启动 n8n..."
        cd "$USER_HOME/n8n-serv00"
        nohup npx n8n start >> "${USER_HOME}/n8n-serv00/n8n/logs/n8n.log" 2>&1 &
        sleep 10
        if check_status; then
            log "日志文件位置: ${USER_HOME}/n8n-serv00/n8n/logs/n8n.log"
        else
